...
function applyBreak(grossHours) {
  ...
  if (grossHours >= 8.5)  return grossHours - 1;   // 8.5h 이상 → 1h 공제
  if (grossHours > 4.5)   return grossHours - 0.5; // P3: 30분 공제
  if (grossHours > 4)     return 4;                // P2: 4h 고정 (휴게 구간)
  return grossHours;                               // P1: 공제 없음
}
