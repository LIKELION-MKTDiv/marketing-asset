...
1. `content-agents-complete.md` 파일을 다운로드합니다.
2. 내 워크스페이스의 `.claude/commands/` 폴더에 넣습니다.

```
my-workspace/
└── .claude/
    └── commands/
        └── content-agents-complete.md
```

3. Claude Code에서 `/content-agents-complete` 입력하면 바로 사용 가능합니다.
...
2. `.claude/commands/content-agents/` 폴더가 워크스페이스에 위치하도록 맞춥니다.
3. `CLAUDE.md`를 워크스페이스 **루트**에 넣습니다.
...
| 파일 | 설명 |
|---|---|
| `content-agents-complete.md` | 모든 에이전트를 하나로 합친 배포용 파일 |
| `CLAUDE.md` | 워크스페이스 루트에 배치 — 톤·타겟·성과 목표 등 공통 상수 자동 로딩 |
| `.claude/commands/content-agents/content-agents.md` | 메인 스킬 파일 |
| `.claude/commands/content-agents/references/` | 에이전트별 상세 지침 파일 |
